...
const EvidenceTab = ({
  ...
  const handleEvidenceStatusChange = (evidenceId, newStatus) => {
    console.log('handleEvidenceStatusChange called');
  ...
  return (
    <Box>
      ...
      >
        {canAddGroup && (
          <StyledButton onClick={handleOpenGroupDialog} startIcon={<AddIcon />}>
            Добавить группу
          </StyledButton>
        )}
        {selectedGroupId && (
          <Box sx={{ display: 'flex', gap: 2 }}>
            {canAddGroup && (
              <StyledButton onClick={handleOpenEvidenceDialog} startIcon={<AddIcon />}>
